      ...
      <el-col :span="24">
        <el-table :data="tableData" style="width: 100%">
          ...
          <el-table-column label="操作">
            <template slot-scope="scope">
              <el-button size="small" @click="toDetail(scope.row.scheme_id)">详情</el-button>
              <el-button
                size="small"
                @click="toEdit(scope.row.scheme_id)"
                v-if="scope.row.line_status==0&&!lineIdentity"
              >编辑</el-button>
              <el-button size="small" @click="toEdit(scope.row.scheme_id)" v-if="lineIdentity">编辑</el-button>
              <el-popover placement="top" width="160" v-model="visible2">
                <p>确定删除？</p>
                <div style="text-align: right; margin: 0">
                  <span @click="deletePlan">
                    <el-button type="primary" size="mini" @click="visible2 = false">确定</el-button>
                  </span>
                </div>
                <el-button
                  v-if="deleteIdentity"
                  @click="getNewId(scope.row.scheme_id)"
                  size="small"
                  slot="reference"
                >删除</el-button>
              </el-popover>
              <el-button
                ...
                :plain="true"
                v-if="scope.row.line_status==0&&lineIdentity"
                @click="upAndLow(scope.row.scheme_id)"
              >上架</el-button>
              <el-button
                size="small"
                :plain="true"
              ...
              <el-button
                ...
                @click="planStick(scope.row.scheme_id)"
              >置顶</el-button>
...
export default {
  ...
  methods: {
    // 上下架
    upAndLow(scheme_id) {
      planUpAndLow(scheme_id).then(res => {
        this.$message({
          message: "操作成功",
          type: "success"
        });
    ...
    deletePlan() {
      deletePlan(this.scheme_id).then(res => {
        this.reload();
      });
    },
    getNewId(scheme_id) {
